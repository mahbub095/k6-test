...
export function familyCardDefault({ token }) {
  ...
  group('08_save_draft_nominee', function () {
    // Nominee gets its own unique name, dob, and verification number
    const nomineeName               = fakeName();
    const nomineeVerificationNumber = generateVerificationNumber();
    const nomineeDob                = (() => {
      const y = 1950 + Math.floor(Math.random() * (2008 - 1950 + 1));
...
// ─── REPORT GENERATOR ────────────────────────────────────────────────────────
// Returns a handleSummary function bound to a specific test name.
...
//
// HTML generation uses the htmlReport() helper from grafana/k6-reporter.
// Install once:  k6 ext install grafana/k6-reporter (k6 < 0.46)
//   or via jslib: import { htmlReport } from 'https://raw.githubusercontent.com/benc-uk/k6-reporter/main/dist/bundle.js';
...
export function makeHandleSummary(testName) {
  return function (data) {
    ...
    console.log(`[summary] Writing reports → ${jsonPath}  |  ${htmlPath}`);

    return {
      [jsonPath]: JSON.stringify(data, null, 2),
      [htmlPath]: htmlReport(data, { title: `Family Card — ${testName.toUpperCase()} Test` }),
