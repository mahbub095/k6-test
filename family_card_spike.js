...
// ─── SPIKE TEST OPTIONS ───────────────────────────────────────────────────────
// Purpose : Verify the system survives a sudden massive traffic surge.
// Pattern : Low baseline → instant spike to peak → drop back → verify recovery.

export const options = {
  stages: [
    { duration: '1m',  target: 50  },  // baseline — normal load
    { duration: '30s', target: 500 },  // spike — sudden 10× surge
    { duration: '3m',  target: 500 },  // hold spike — observe stability
    { duration: '30s', target: 50  },  // drop — sudden return to baseline
    { duration: '2m',  target: 50  },  // recovery hold — verify system recovers
    { duration: '1m',  target: 0   },  // ramp-down
  ],
  thresholds: {
    // Spike tests use relaxed thresholds — some degradation is expected
