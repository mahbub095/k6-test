...
export default function () {
  ...
  // ── Step 1: Request OTP ──────────────────────────────────────────────────
  group('01_request_otp', function () {
    const payload = JSON.stringify({
      device_token:    'c5526b12-cd80-4dff-9d1f-f173bb3f7c69',
      username:        'enumghatail',
      password:        'Password#1',
      otp:             '',
      captchaValue:    '27',
      captcha_token:   '$2y$12$07dsdA9l1G0gRIqn.7pe4uSvy9p35F8pl3K2Pe3pudImDJUv4WzAW',
      captcha_value:   '27',
    });

    const res = http.post(
      `${BASE_URL}/api/v1/family-card/login/otp?lang=bn`,
      payload,
      {
        headers: {
          'Accept':           'application/json, text/plain, */*',
        ...
        timeout: '15s',
        tags: { name: 'request_otp' },
      }
    );

    check(res, {
      'OTP request: status 2xx': r => r.status >= 200 && r.status < 300,
      'OTP request: response time < 2000ms': r => r.timings.duration < 2000,
    });
  });
  ...
  group('02_login', function () {
    ...
    const res = http.post(
      `${BASE_URL}/api/v1/family-card/login?lang=bn`,
      payload,
      {
        headers: {
          'Accept':           'application/json, text/plain, */*',
  ...
  // ── Step 3: Get application page data ────────────────────────────────────
  group('03_get_application_page_data', function () {
    const res = http.get(
  ...
  // ── Step 4: Get application counts ───────────────────────────────────────
  group('04_get_application_counts', function () {
    const res = http.get(
  ...
  // ── Step 5: Location lookups ──────────────────────────────────────────────
  group('05_location_lookups', function () {
    // District
  ...
  // ── Step 6: Save draft – personal info ───────────────────────────────────
  let draftId = null;

  group('06_save_draft_personal', function () {
    const formData = {
      lang:                'bn',
      profession:          '109',
      sub_program_id:      '24',
  ...
  group('07_media_uploads', function () {
  ...
  // ── Step 8: Save draft – photos (final submit with uploaded file paths) ───
  group('08_save_draft_photos', function () {
    const formData = {
      lang:                'bn',
      program_id:          '24',
      sub_program_id:      '24',
  ...
  // ── Step 9: Final application counts refresh ──────────────────────────────
  group('09_final_counts_refresh', function () {
    const res = http.get(
      `${BASE_URL}/api/v1/family-card/applications/counts?lang=bn`,
      {
        headers: authGetHeaders(token),
