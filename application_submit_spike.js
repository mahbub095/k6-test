...
export const options = {
    scenarios: {
        application_submit_spike: {
            executor: 'ramping-vus',
            stages: [
                { duration: '1m',  target: 50  },  // baseline — normal idle
                { duration: '30s', target: 600 },  // spike — sudden 12× surge
                { duration: '3m',  target: 600 },  // hold spike — observe stability
                { duration: '30s', target: 50  },  // spike drop — sudden return to normal
                { duration: '2m',  target: 50  },  // post-spike hold — verify recovery
    ...
    thresholds: {
        // Spike thresholds allow high error rates during the spike window
        media_upload_duration:  [{ threshold: 'p(95)<30000', abortOnFail: false }],
        ...
        http_req_duration: [
            { threshold: 'p(90)<20000', abortOnFail: false },
...
export default function () {
    ...
    group('Step 1 - GET Online Application Page', () => {
        const res = http.get(`${PAGE_URL}/online-application`, {
            ...
            timeout: '60s',
            tags:    { step: '01_get_page', test: 'spike' },
        });
        requestCount.add(1);
        const ok = check(res, {
    ...
    group('Step 2 - GET Application Page Data', () => {
        const res = http.get(`${API_URL}/api/v1/global/getApplicationPageData?lang=bn`, {
            headers: API_HEADERS,
            timeout: '60s',
            tags:    { step: '02_page_data', test: 'spike' },
        });
        requestCount.add(1);
    ...
    group('Step 3 - GET Disabled Areas', () => {
        const res = http.get(`${API_URL}/api/v1/global/online-application/disabled-areas/8?lang=bn`, {
            headers: API_HEADERS,
            timeout: '60s',
            tags:    { step: '03_disabled_areas', test: 'spike' },
        });
        requestCount.add(1);
    ...
    group('Step 4 - GET Captcha', () => {
        const res = http.get(`${API_URL}/api/v1/captcha?lang=bn`, {
            headers: API_HEADERS,
            timeout: '60s',
            tags:    { step: '04_captcha', test: 'spike' },
        });
        requestCount.add(1);
    ...
    group('Step 5 - POST Media Upload', () => {
        const res = http.post(
            ...
            { headers: API_HEADERS, timeout: '120s', tags: { step: '05_media_upload', test: 'spike' } }
        );
        requestCount.add(1);
    ...
    if (!uploadedImagePath) {
        console.error(`VU ${__VU} [spike] — aborting: no image path from media-upload`);
        return;
    }
    ...
    group('Steps 6-19 - Lookup GETs', () => {
        ...
        const responses = http.batch(
            lookupUrls.map(url => ({
                method: 'GET',
                url,
                params: { headers: API_HEADERS, timeout: '60s', tags: { step: '06_lookups', test: 'spike' } },
            }))
        );
    ...
    group('Steps 20-22 - GET Check Duplicate Account', () => {
        const dupUrl =
            ...
            `&ignore_id=&lang=bn`;
        const responses = http.batch([
            { method: 'GET', url: dupUrl, params: { headers: API_HEADERS, timeout: '60s', tags: { step: '20_check_duplicate', test: 'spike' } } },
            { method: 'GET', url: dupUrl, params: { headers: API_HEADERS, timeout: '60s', tags: { step: '21_check_duplicate', test: 'spike' } } },
            { method: 'GET', url: dupUrl, params: { headers: API_HEADERS, timeout: '60s', tags: { step: '22_check_duplicate', test: 'spike' } } },
        ]);
        requestCount.add(3);
        responses.forEach((res, i) => {
    ...
    group('Step 23 - POST Registration', () => {
        if (!captchaToken) {
            console.warn(`VU ${__VU} [spike] — skipping POST: no captcha_token`);
            errorRate.add(1);
            return;
        }
        ...
        const res = http.post(
            ...
            formData,
            { headers: API_HEADERS, timeout: '120s', tags: { step: '23_post_registration', test: 'spike' } }
        );
        requestCount.add(1);
        ...
        console.log(`VU ${__VU} [spike] — registration HTTP ${res.status} | ${res.timings.duration}ms`);
        if (!ok) console.error(`VU ${__VU} [spike] FAILED — body: ${res.body}`);
    });
    sleep(10);
}
