...
export default function () {
    ...
    group('Step 1 - GET Page', () => {
        ...
        const ok = check(res, { 'step01 → 200': r => r.status === 200 });
        if (!ok) errorRate.add(1);
    });

    // ── Step 2 — GET application page data ───────────────────────────────────
    group('Step 2 - GET Page Data', () => {
        ...
        requestCount.add(1);
        classifyResponse(res);
        const ok = check(res, { 'step02 → 200': r => r.status === 200 });
        if (!ok) errorRate.add(1);
    });
    ...
    group('Step 3 - GET Disabled Areas', () => {
        ...
        requestCount.add(1);
        classifyResponse(res);
        const ok = check(res, { 'step03 → 200': r => r.status === 200 });
        if (!ok) errorRate.add(1);
    });
    ...
    group('Step 4 - GET Captcha', () => {
        ...
        const ok = check(res, { 'step04 → 200': r => r.status === 200 });
    ...
    group('Steps 6-19 - Lookup GETs', () => {
        ...
        const responses = http.batch(
            lookupUrls.map(url => ({ method: 'GET', url, params: LOOKUP_PARAMS }))
        );
    ...
    group('Steps 20-22 - Duplicate Check', () => {
        ...
        const responses = http.batch([
            { method: 'GET', url: dupUrl, params: { headers: API_HEADERS, timeout: '10s', responseType: 'none', tags: { step: '20_dup_check', test: 'breakpoint' } } },
            { method: 'GET', url: dupUrl, params: { headers: API_HEADERS, timeout: '10s', responseType: 'none', tags: { step: '21_dup_check', test: 'breakpoint' } } },
            { method: 'GET', url: dupUrl, params: { headers: API_HEADERS, timeout: '10s', responseType: 'none', tags: { step: '22_dup_check', test: 'breakpoint' } } },
        ]);
        requestCount.add(3);
        responses.forEach((res, i) => {
            classifyResponse(res);
