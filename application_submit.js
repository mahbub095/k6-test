...
const PAGE_URL = 'https://stage.bhata.gov.bd';       // BASE_URL_1 — page server
const API_URL = 'https://stage-api.bhata.gov.bd';        // BASE_URL_2 — API server
const GATEWAY_URL = 'https://gateway.bhata.gov.bd';   // BASE_URL_3 — media gateway

const BEARER_TOKEN = __ENV.BEARER_TOKEN || '';

// Image pre-wrapped at init time
...
const RAMP_UP_STAGES = [
    ...
    { duration: '2m', target: 5000 },   // 100 → 500
    { duration: '5m', target: 1000 },  // 500 → 1,000
];
...
// ---------------------------------------------------------------------------
// Helper: generate a UUID v4 — used as media_token (client-generated, sent
// with the upload and repeated in registration so the server can match them)
// ---------------------------------------------------------------------------
...
function buildApplicant() {
    ...
    return {
        verification_number: `${prefix}${suffix}`,
        account_number,
        date_of_birth,
        age,
...
export default function () {
    ...
    console.log(`VU ${__VU} — BRS: ${applicant.verification_number} | acc: ${applicant.account_number}`);
    ...
    // Steps 6–19 — Batched lookup GETs  (mirrors JMX order)
    ...
    group('Step 23 - POST Registration', () => {
        ...
        const formData = {
            ...
            // Media token from upload step
            media_token: mediaToken,

            // Image — server path returned from media-upload (validated above)
